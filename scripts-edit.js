		...
		userFilesDropzone: {
			...
			initFileUploader: function(base, target) {
				...
				var onyxDropzone = new Dropzone(target, {
					url: ($(target).attr("action")) ? $(target).attr("action") : "../../file-upload-edit.php", // Check that our form has an action attr and if not, set one here
					 params: {
				...
				onyxDropzone.on("totaluploadprogress", function (progress) {
					...
					$('#disa').prop("disabled", true);  // TheMornStar has this type of element.
				});
				...
				onyxDropzone.on('removedfile', function (file) {
					...
					$.ajax({
						type: "POST",
						url: ($(target).attr("action")) ? $(target).attr("action") : "../../file-upload-edit.php",
						data: {
				...
				onyxDropzone.on("success", function(file, response) {
					...
					// Something to happen when file is uploaded // TheMornStar Edit has this 
				});
			},
			dropzoneCount: function() {
				var filesCount1 = $("#previews > .dz-success.dz-complete").length ;
				var filesCount2 =  $("#previews-old > .dz-success.dz-complete").length;
				var filesCount = filesCount1 + filesCount2 ;
				return filesCount;
			},
