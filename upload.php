...
if(move_uploaded_file($_FILES["file1"]["tmp_name"], "upload/" . $newfilename)){
   ...
   $event_last_id = $_GET["appid"]; 
	$filename = $_FILES["file1"]["name"];
	$file_basename = substr($filename, 0, strripos($filename, '.')); // get file extention
	$file_ext = substr($filename, strripos($filename, '.')); // get file name
	$filesize = $_FILES["file1"]["size"];
	$allowed_file_types = array('.jpg','.mp4','.docx','.mp3','.jpeg','.png','.pdf');	
	...
	{	
		...
		{		
			...
			mysqli_query($conn,"INSERT INTO `event_file`(`event_fileid`, `event_filename`, `event_fileurl`, `event_file_unique`) 
			VALUES ('', '".mysqli_real_escape_string($conn,$filename)."','".mysqli_real_escape_string($conn,$newfilename)."','$event_last_id')");
			echo "SUCCESS:  ";
            echo "<a target='_blank' style='color:blue' href='upload/".$newfilename."'>".$filename."</a>";	
