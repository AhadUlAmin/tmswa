...
$check_news = mysqli_query($conn,"select * from `article` where `article_identify` = '" . mysqli_real_escape_string($conn,$ms_uid) . "'");


if (isset($ms_uid) && !empty($ms_uid) && mysqli_num_rows($check_news) > 0)
{
$get_checked_contents_from_the_database = mysqli_fetch_array(mysqli_query($conn,"select * from `article` where `article_identify` = '".mysqli_real_escape_string($conn,$ms_uid)."'"));
